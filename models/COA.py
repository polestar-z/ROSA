...
class COA(BaseModel):
    ...
    For more details, you may refer to `COA<https://docs.dgl.ai/en/0.8.x/generated/dgl.nn.pytorch.conv.COAConv.html>`__
        ...
        if hasattr(hg, 'ntypes'):
            ...
            with hg.local_scope():
                ...
                for l in range(self.num_layers):             
                    ...
                    h = self.coa_layers[l](g, h, g.ndata['_TYPE'], g.edata['_TYPE'], presorted=True)
            ...
            print("\n------ Mini-batch Training Debug ------")
            print(f"h_dict type: {type(h_dict)}")
            print(f"h_dict shape: {h_dict.shape if isinstance(h_dict, torch.Tensor) else 'Not a tensor'}")               
                      
            for layer, block in zip(self.coa_layers, hg):
                print(f"\n--- Processing Layer {layer} ---")
                print(f"Block types (ntypes): {block.ntypes}")
                print(f"Block edge types (etypes): {block.etypes}")
                print(f"Block node count: {block.num_nodes()}")             
                print(f"Block edge count: {block.num_edges()}")             
...
class COAConv(nn.Module):
    ...
    def forward(self, g, x, ntype, etype, *, presorted=False):
        ...
        with g.local_scope():                                                          
            ...
            """
                self.linear_k(x_src, srcntype, presorted) output shape: (N_src, num_heads * head_size)
                .view(-1, self.num_heads, self.head_size) reshapes to: (N_src, num_heads, head_size)
