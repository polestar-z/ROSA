...
@register_model('HGT_Multi')
class HGT_Multi(BaseModel):
    r"""
    HGT (Heterogeneous Graph Transformer) adapted for multi-label classification
    ...
    def build_model_from_args(cls, args, hg):
        """
        Build HGT_Multi model from arguments
        ...
        if hasattr(args, 'category'):
            ...
            print(f"[HGT_Multi] Auto-detected target node type: '{target_ntype}'")
    ...
    def __init__(self, in_dim, out_dim, num_heads, num_etypes, ntypes,
        ...
        """
        super(HGT_Multi, self).__init__()
        self.num_layers = num_layers
        self.ntypes = ntypes
        self.target_ntype = target_ntype

        print(f"[HGT_Multi] Initializing model with target node type: '{target_ntype}'")
        print(f"[HGT_Multi] Output dimension (num_classes): {out_dim}")
        ...
        print(f"[HGT_Multi] Model initialized with {num_layers} layers")

    def forward(self, hg, h_dict):
        """
        Forward pass of HGT_Multi
...
class HGTConv(nn.Module):
    ...
    This is a copy of the HGTConv from HGT.py to ensure independence.
    Implements the core HGT message passing mechanism.
    """
