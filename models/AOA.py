...
class AOA(BaseModel):
    ...
    graph AOA from paper `Heterogeneous Graph Attention Network <https://arxiv.org/pdf/1903.07293.pdf>`__..
    Because the original AOA implementation only gives the preprocessed homogeneous graph, this model
    could not reproduce the result in AOA as they did not provide the preprocessing code, and we
    ...
    def forward(self, g, h_dict):
        ...
        """
        out_dict = {}
        for ntype, aoa in self.mod_dict.items():                 
        ...
        for ntype, h in out_dict.items():
            print(f"[AOA] ntype = {ntype}, output shape = {h.shape}")         
