...
@register_model('HAN_Multi')
class HAN_Multi(BaseModel):
    r"""
    HAN model for multi-label node classification.

    This is a modified version of HAN that supports multi-label classification on heterogeneous graphs.
    The key difference from the original HAN is that this model only returns outputs for the target node type
    ...
    def __init__(self, ntype_meta_paths_dict, in_dim, hidden_dim, out_dim, num_heads, dropout, target_ntype):
        """
        Initialize HAN_Multi model.

        Key differences from original HAN:
        - Stores target_ntype
        - Only creates _HAN module for the target node type
        """
        super(HAN_Multi, self).__init__()
    ...
    def forward(self, g, h_dict):
        ...
        Key difference from original HAN: Only returns output for target node type.
        ...
        if self.target_ntype in self.mod_dict:
            han = self.mod_dict[self.target_ntype]
...
class _HAN(nn.Module):
    """
    Internal HAN module for a single node type.

    This class remains unchanged from the original HAN implementation.
    """
    ...
    def forward(self, g, h_dict):
        """
        Forward propagation through all HAN layers.
...
class HANLayer(nn.Module):
    """
    HAN layer with meta-path based attention.
